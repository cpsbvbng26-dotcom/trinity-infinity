...
**残った事実は、すべて既知の結果の再発見です。**この系列が独自に見つけたものは
一つもありません。以下は対応表であって、謙遜ではありません。

| この系列が述べたこと | すでにある名前 |
| --- | --- |
| 作用素 `x ← DQx + (I − D)p` | **アフィン反復**（`x ← Mx + c` の形）。数値線形代数でいう**定常反復法**で、Jacobi 法・Gauss–Seidel 法と同じ枠に入る |
| 縮小写像なので唯一の不動点へ幾何収束する | **バナッハの不動点定理**（1922）。系列は正しく引用しており、ここは再発見ではなく利用である |
| 縮小定数は `maxᵢ aᵢ` | 対角行列と置換行列の積のノルム。初等的な計算 |
| 任意の `n ≥ 2` で同じ証明が通る（III） | 発見ではなく、**`n = 3` という区別が最初から無かったことの確認** |
| 巡回置換が自己逆にならない最小の `n` が 3 | 置換群の初等的な事実 |

**「作用素ノルムが 1 未満」を収束の条件として述べていた点も、標準的な結果の
言い直しに過ぎません。**`x ← Ax + b` が任意の初期値から収束する必要十分条件が
`ρ(A) < 1` であることは、数値線形代数の教科書に載っています
（[trinity-operator](https://github.com/cpsbvbng26-dotcom/trinity-operator) で実装しました）。
そこで使う「`ρ(A) < 1` なら A が縮小になるノルムが存在する」も、
Ostrowski / Householder の古典的な結果です。

つまり**訂正の側も再発見**です。誤りを見つけたことに独自性があるのではなく、
教科書に書いてあることと突き合わせたら合わなかった、というだけです。

**この表を覆すには**、上の五行のどれか一つについて「これは既存の文献に無い」と
示してください。一件で覆ります。

この系列の記録としての意味は、数学の中身ではなく、**何を撤回し、何が直せずに
残ったかのほう**にあります。
