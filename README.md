...
`claims_audit.py` (32), `check_errata.py` (94, needs pypdf), `check_route.py` (112).
...
| [`verification/check_errata.py`](verification/check_errata.py) | **正誤表が紙面について述べていることは、紙面と合うか。** PDF から文字を取り出して突き合わせる | 94 |
...
突き合わせる（94 項目）。E1・E2 の引用も、名乗る件数も、未解決の項目が
