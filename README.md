...
> **ただし、一つ先に断ります。**初稿は著者が導いたものではありません。著者が
> 持っていたのは `Ⅲ∞` が指定する範囲 —— 三つのものを果てまで反復する —— までで、
> **写像も不動点も収束も定理も、当てはめたのは言語モデルです**
> （[ERRATA E4](ERRATA.md)）。**「独立して到達した」という意味での再発見は、
> この系列には当てはまりません。**当てはまるのは「言語モデルが標準的な構成を
> 出力し、それが新規の枠組みとして公開された」という形です。以下の分類は、
> **何が既知だったか**についてのものであって、誰が到達したかではありません。
