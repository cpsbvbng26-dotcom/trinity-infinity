...
`claims_audit.py` (32), `check_errata.py` (108, needs pypdf), `check_route.py` (132).
...
**取り違えたのは、その講義がちょうど試験で問う一点である。**
**だから、同じ取り違えを二度と通さない検査に落とせた。**
...
**ただし、「ロマン」を事前の自己診断として読むのは、回顧的な解釈である。**
題名に置かれたその語は、普通は賛辞として読まれる。**着地を知ってから題名を読み直している。**
等級は `紙面` ではなく、**回顧的解釈・検証不能**である。

**そして、題名の一語が当たっていたことは、本を救わない。**本そのものは撤回されており、
...
| [`verification/check_route.py`](verification/check_route.py) | **[ROUTE.md](ROUTE.md) の九段階は、他の文書と食い違わないか。** | 132 |
