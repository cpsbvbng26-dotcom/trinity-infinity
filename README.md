...
`claims_audit.py` (32), `check_errata.py` (108, needs pypdf), `check_route.py` (139).
...
残ったのは、一つの作用素についての一つの事実です。`Q` が n 巡回置換で `D` が対角の
とき、`(DQ)ⁿ = (∏ᵢ aᵢ)·I` がちょうど成り立ちます。だから収束を決めているのは係数の
相乗平均であって、最大値でも作用素ノルムでもありません。

この事実は正しく、機械で確かめられ、そして学部 2〜3 年の演習問題の水準です。
取り違えたのは、その講義がちょうど試験で問う一点です。
だから、同じ取り違えを二度と通さない検査に落とせました。
直しに使った道具は、大学院の線形システム論と行列解析に属します。
いちばん近い学科は、数値解析ではなく線形システム論・現代制御です。

道具が大学院の側にあっても、直された中身は演習問題のままです
（[どの水準の数学か](#どの水準の数学か)）。

記号 `Ⅲ∞` のうち、数学的な内容がある側は `Ⅲ`（＝ 三）でした。その側が、意味を
持たないと証明されました。残ったのは `∞`（反復して極限をとる）で、これは対象の型を
言っているだけで、定理を含みません。
...
英文の定義と、そこに入れる四つの条項は [`ERRATA.md`](ERRATA.md) の `E8` にあります。
...
[`ERRATA.md`](ERRATA.md) の `E8` が重大度・高で立っているあいだ、この四文は帰属の側で立ちません。
既知のモデル（Friedkin–Johnsen）の特殊例を、自分の貢献として述べていることになります。
枠組みについては何も偽にしていません。貢献については、空白ではありません。

だから「真であることに尽きる」は、帰属の修正が入ったあとに、その用途を全部果たす。
修正が入るまで、下限と上限が一致しているとは、現在形では書けません。

真であることは、論文の下限であって上限ではありません。どの論文もそこは満たしている
はずのものです。

題名も、同じ割れ方をしています。撤回した一般向けの本の題は
『**グランドセオリーというロマン** —— **トリニティインフィニティフレームワーク**』だった
（[登録簿](https://github.com/cpsbvbng26-dotcom/self-correction) の `BK-001`）。
...
ただし、「ロマン」を事前の自己診断として読むのは、回顧的な解釈です。
題名に置かれたその語は、普通は賛辞として読まれます。着地を知ってから題名を読み直しています。
等級は `紙面` ではなく、**回顧的解釈・検証不能**です。

そして、題名の一語が当たっていたことは、本を救いません。本そのものは撤回されており、
何を主張していたかは確かめられない（`BK-001`）。

この系列の価値は、枠組みにではなく、枠組みが縮んでいく過程が消さずに残っている
ことのほうにあります。そこも自分では測れない
（[登録簿の `ST-002`](https://github.com/cpsbvbng26-dotcom/self-correction)）。

経緯は [どの水準の数学か](#どの水準の数学か)と
[結局、何をしてどう終わったか](#結局何をしてどう終わったか)に、
順序は [ROUTE.md](ROUTE.md) にあります。

この判定を保ったまま、arXiv へ出すことを著者が決めた（2026年9月9日）。
出す形と、却下される見込みを先に書いたものを [ARXIV.md](ARXIV.md) に置いてあります。
新しい結果が無いという上の一行は、その決定によって変わりません。

---

三本の系列です。枠組みを提示した系列ではありません。枠組みが自分を検証にかけて縮んでいく系列です。
...
どちらも厳しい条件ではありません。本来すでに満たされているべきものです。この系列が記録として
意味を持つのは、それを満たさなかった部分がどれだけあったかのほうです。
...
同一の本文です。正は Zenodo の [10.5281/zenodo.22058777](https://doi.org/10.5281/zenodo.22058777) です。
SSRN の選別は主題と体裁を見るものであり、査読ではありません。

本文は PDF のままです。Markdown への書き起こしはしていません。数式の下付き文字が抽出時に
別行へ分解されるため、起こすには数式を組み直す必要があり、それは論文を変えることになります。
...
n = 3 が区別されるのは一点です。巡回置換がそれ自身の逆写像にならない最小の n であること。
...
そこへ至る九段階は [ROUTE.md](ROUTE.md) に刻んです。
...
再発見そのものは失敗ではありません。どう置いたかで意味が決まる。
...
再発見だと書いた記録です。価値は数学の中身ではなく、材料としての形の側にあります。

## どの水準の数学か

残ったものは、学部 2〜3 年の演習問題です。
犯した誤りは、その講義が試験で問う取り違えです。
直しに使った道具は、大学院の線形システム論と行列解析に属します。
いちばん近い学科は、数値解析ではなく線形システム論・現代制御です。
順位は書きません。確かめようも覆しようもないからです。
...
三つめだけが上にはみ出しています。それで全体の水準が上がるわけではありません。
直した道具が上でも、直された中身は演習問題のままです。壊れたものを直すのに、
それより上の道具が要ったというだけです。

### 大学院に入る部分

二つあります。どちらも、学部の講義が扱う範囲の外にあります。

一つめは、非正規行列の過渡的な増大です。反例 `Q = [[1,40],[0,1]]` は
`ρ = 0.5` なのに `‖A‖₂ = 20.01` で、誤差は第 2 段で初期値の 11.8 倍に膨らんでから
減っていく。学部の講義は `ρ < 1` なら収束すると述べるところまでで、この山の高さは
扱いません。山を出すには行列が正規でない場合を見る必要があり、それは擬スペクトルの
主題です（Trefethen & Embree 2005）。

二つめは、縮小になるノルムの構成です。`ρ < 1` から離散リアプノフ方程式
`P − AᵀPA = W` を解いて重み付きノルムを作り、`κ = γ√(1 − 1/λmax(P))` が上界ではなく
`‖A‖_P` そのものであることを示しました。Horn & Johnson §5.6 と、下に挙げた制御の講義の
水準です。

ただし、これで全体が大学院の水準になるわけではありません。大学院の道具を使ったのは、
学部の演習問題を取り違えたのを直すためです。道具の水準と、直された中身の水準は
別です。
...
犯した誤りも、その授業が教えようとしている落とし穴そのものです。
ノルムが 1 未満なら収束します。これは正しい。逆は言えません。`ρ(A) ≤ ‖A‖` の不等号が
どちら向きかを取り違えると、十分条件を必要十分条件だと思い込みます。教科書がこの不等式を
先に置くのは、その取り違えを防ぐためです。そこで転んだ。

### どの学科で扱うか

一つの学科に収まりません。同じ対象が、四つの場所で別の名前で扱われています。
...
いちばん近いのは三つめです。この作用素が扱っているのは、三つの要素が反復して
一つに収束する系であり、離散時間の線形システムの漸近安定性そのものです。壊れた証明を
直すのに使った離散リアプノフ方程式 `X − AᵀXA = Q` も、そこの標準道具です。
論文は数値解析の言葉で書かれていたが、対象は制御の対象でした。

世界大学ランキングは、この件の軸になりません。上の内容は、数値解析や線形システム論を
開講している大学なら順位に関係なく出てくる。順位で変わるのは進度と周囲の科目であって、
`ρ(A) ≤ ‖A‖` が出るかどうかではありません。「順位 N 位の大学の演習に相当する」は、確かめようも
覆しようもありません。だから書きません。
...
始まり。記号 `Ⅲ∞` がありました。三つのものを、果てまで反復します。それだけです。
写像も、空間も、距離も、収束も、記号からは出ない（[経路](ROUTE.md#記号が決めたこと決めなかったこと)）。

やったこと。その記号を言語モデルに渡し、出てきた構成を論文として公開しました。
中身は `x ← DQx + (I − D)p` という一次の反復で、収束は縮小写像から従うと書きました。
そこにゲーム理論・機械工学・AI アライメント・気候政策への応用まで載せました。

壊れた順。

1. 三人ゲームの利得表が 4 セルしかありません。三人・二戦略なら 8 通りある
2. 「98.7% の試行で確認」に、再現できるコードも乱数種も無い
3. 離散の反復に、連続時間のリアプノフ微分と LaSalle の原理を当てていた
4. 応用の主張に根拠が無い
5. `n = 3` に数学的な意味がありません。外から言われる前に、三本目で著者が自分で否定した
6. 収束を決めているのはノルムではなくスペクトル半径でした。反例は
   `Q = [[1,40],[0,1]]` で `ρ = 0.5`、`‖A‖₂ = 20.01`、誤差は第 2 段で初期値の 11.8 倍に膨らむ

残ったもの。`Q` が n 巡回置換で `D` が対角のとき、`(DQ)ⁿ = (∏ᵢ aᵢ)·I` がちょうど成り立ちます。
だから収束を決めているのは係数の相乗平均であって、最大値でも作用素ノルムでもありません。
これは正しく、機械で確かめられ、そして教科書の演習問題です。

記号の行方。`Ⅲ`（＝ 三）は、記号のうち数学的な内容がある側でした。
その側が、意味を持たないと証明されました。残ったのは `∞`（反復して極限をとる）のほうで、
これは対象の型を言っているだけで、定理を含みません。

枠組みは残りませんでした。残ったのは、一つの作用素についての一つの事実と、
それが壊れていく過程の記録です。消していない理由は
[なぜ消さずに置いてあるのか](#なぜ消さずに置いてあるのか)にあります。

## 新しい数学は含まれていない

以上から結論が出ます。残った事実に、既存の文献に無いものは一つもありません。訂正の側も
同じです。`x ← Ax + b` が任意の初期値から収束する必要十分条件が `ρ(A) < 1` です
ことも（Varga 2000）、`ρ(A) < 1` なら `A` が縮小になるノルムが存在することも
（Ostrowski / Householder 1964; Horn & Johnson 2013 §5.6）、教科書に載っています。

誤りを見つけたことに独自性があるのではありません。教科書と突き合わせたら合わなかった、
というだけです。

この結論は、上の五行のどれか一つについて「これは既存の文献に無い」と示せば覆ります。
一件で覆ります。
...
改訂前の草稿が主張していて、改訂版に残らなかったものです。各論文が自ら列挙しています。

ここでいう「撤回」は、改訂で落とした主張を指します。出版上の retraction ではありません。初版のレコードは生きており、この記録の範囲に撤回通知はありません。
...
著者が販売停止にしました。著者はこの書き物の正式な種別を「マニフェスト」と述べています。

上の表と、この一件は性質が違います。上は改訂で落とした主張であり、初版のレコードは
Zenodo に生きたまま残っています。だから何を撤回したかを第三者が確かめられます。
書籍については、それができません。残っているのは著者の言葉だけで、ISBN も ASIN も
記録していません。この記録で唯一「消した」ものです。

確かめられないことが三つあります。何を主張していたか。プレプリント側で撤回した応用
（AI アライメント・気候政策・ガバナンス）が世界観として残っていたか。そして、出したのが
2026年8月の改訂の前か後か。前なら初版と一貫し、後なら食い違います。どちらかは分かりません。

販売停止は著者の申告です。作業環境から Amazon に届かないので、こちらでは確かめていません。
登録簿の [`BK-001`](https://github.com/cpsbvbng26-dotcom/self-correction/blob/main/REGISTER.md)
に記録してあります。外部の評価がこの本を挙げたことで、記録に無い公開物があると分かりました。
...
| [`verification/check_route.py`](verification/check_route.py) | [ROUTE.md](ROUTE.md) の九段階は、他の文書と食い違わないか。 | 139 |
...
上の二本は NumPy のみを必要とし、乱数種を固定しています。47 項目すべてが通ります。
**GitHub Actions が push ごとに四本すべてを実行している** —— 上のバッジが、いま通っているかどうかです。
...
なお、各論文は検証スクリプトを同梱していると謳っているが、**そのスクリプトは存在しません**。
配布物にも著者の手元にも残っていません（[ERRATA.md](ERRATA.md) の E3）。謳っているのは謝辞だけでは
なく、三本で合計 **6 箇所**です。ここにある四本は、その代用ではなく別に書いたものです。
...
やる。判定に推論を使いません。あるか、無いか、一致するか、しないか。
...
改訂版 Series I を `10.5281/zenodo.17173703` で引用しています。 これは訂正の対象です
...
写像と定理を当てはめる形で、**言語モデル（ChatGPT / OpenAI）が生成したもの**です。三本の PDF にはこの記載が
ありません。紙面は改訂への Claude の関与しか述べておらず、初稿の出所に触れて
いません。凍結された PDF は直せないため、[ERRATA.md](ERRATA.md) の `E4` に
記録しています。当時、自己登録のプレプリントに開示義務は無く、規則違反では
ありません。

このリポジトリについて。 README の構成と本文、`ERRATA.md`、`CITATION.cff`、および
`verification/` のスクリプトは、Claude Code（Anthropic）を用いて作成し、実行しました。**論文の PDF には手を入れていない** ——
...
[canonical-sources](https://github.com/cpsbvbng26-dotcom/cpsbvbng26-dotcom/blob/main/docs/canonical-sources.md)
に固定してあります。訂正の記録そのものは
[self-correction](https://github.com/cpsbvbng26-dotcom/self-correction) の登録簿にあります。
