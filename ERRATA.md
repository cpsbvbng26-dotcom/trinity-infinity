...
```
Friedkin–Johnsen:  x(t+1) = ΛW(t) x(t) + (I − Λ) u
この作用素:         x     ← D Q    x    + (I − D) p
```

| Friedkin–Johnsen | この作用素 | |
| --- | --- | --- |
| `Λ = diag(λᵢ)`、`λᵢ ∈ [0,1]` は社会的影響の受けやすさ | `D = diag(aᵢ)`、`aᵢ ∈ (0,1)` は混合率 | 対角行列 |
| `W` は影響行列（行確率的） | `Q` は巡回置換行列 | **置換行列は二重確率的。`W` の特殊例である** |
| `u` は各主体の先入見（prejudice） | `p` は固定した基準点 | 錨 |
| `1 − λᵢ` が先入見への傾き | `1 − aᵢ` が基準点への傾き | 同じ位置 |

**Series III の Theorem 1 は、`W` を巡回置換に限った既知の収束結果である。**
Series I 第2節の Remark 1 が置く閉形式 `T* = (1−α)(I−αP)⁻¹p` も、
このモデルの平衡解 `x* = (I − ΛW)⁻¹(I − Λ)u` を `Λ = αI` にした形である。
収束を `ρ < 1` で決めるところも同じである。

> Friedkin, N. E., & Johnsen, E. C. (1990). Social influence and opinions.
> *The Journal of Mathematical Sociology*, 15(3–4), 193–205.
> DOI: [10.1080/0022250X.1990.9990069](https://doi.org/10.1080/0022250X.1990.9990069)

Friedkin–Johnsen は DeGroot モデル（影響だけで先入見が無い）に先入見を足したものである。
**この作用素が持っている `(I − D)p` の項が、まさにその追加分に当たる。**
...
**どこまで確かめたか。**モデルの式・`Λ` と `u` の意味・DeGroot との関係・書誌は、
**検索で確認した。原典には当たっていない**（作業環境から本文に届かない）。
**根拠の等級は `証言` ではなく、原典未読の確認である。**
`ρ` で収束を決める点も同じだと読めるが、Friedkin–Johnsen の原論文が
収束定理をどの形で述べているかは、本文を見ていない。
