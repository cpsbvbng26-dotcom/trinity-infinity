...
巡回置換は `n` 次元、一般は `n(n−1)` 次元である。`σ` を変えても
`(n−1)!` 枚の `n` 次元の面が並ぶだけで、次元は上がらない。
`n ≥ 3` では、巡回の像は一般の像のなかで測度零である。

落ちている `n(n−2)` 次元が、誰が誰にどれだけ影響するかである。
`W` を巡回置換に限るとは、**各主体がちょうど一人の隣人だけを、重み 1 で聴く輪**に
限ることである。FJ を社会学たらしめている当のものが、そこで落ちる。

### 一点の均衡では、区別がつかない

そして、これが誤りの続いた理由の一部である。

`Q` が置換なら `DQ` の各行は非零要素を一つしか持たない。よって不動点は
`x*ᵢ = aᵢ x*_{σ(i)} + (1 − aᵢ) pᵢ` を満たす。必要条件になる。
...
一本の軌道だけを見ているかぎり、特殊例と一般とは区別がつかない。

ただし、これは弁明ではない。Series I は第1節で合意形成動学を名指ししている。
文献を引いていれば、軌道を見るまでもなかった。区別がつかなかったのは
軌道の側であって、文献の側では最初から見えていた。
...
FJ を社会学たらしめている当のもの、実証計画が測定するその対象 —— を全部落としている。
