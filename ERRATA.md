...
つまり、**謝辞の記述は事実ではありません。** 三本の論文の数値を、記述どおりの手段で確かめることは
できません。

**このリポジトリでの扱い。** 書き直したものを「これがそれです」と称して置くことはしません。
代わりに [`verification/claims_audit.py`](verification/claims_audit.py) を**別に**書きました。
**三本の紙面に印字されている数値を一つずつ計算し直し、印字された値と並べて表示します。**
31 項目すべてが一致します。

**これは同梱スクリプトの代わりではありません。** 独立に書かれた突き合わせです。元のスクリプトが
何を計算していたかは、もう誰にも分かりません。**分かるのは、紙面の数値が計算し直しても出る、
ということだけです。**
...
三本の開示文は、**改訂**について Claude の関与を述べています。

> This revision was prepared with drafting and mathematical-formalization assistance from Claude (Anthropic)

**「revision（改訂）」と書いてあります。**

**初稿がどう作られたかは、三本のどこにも書かれていません。**

### 実際の経緯

**著者が持っていたのは記号 `Ⅲ∞` までです。**その記号に数式を当てはめたのは
言語モデル（ChatGPT / OpenAI）で、その出力が 2025年10月の初版になりました。
...
三本の開示文は Claude しか挙げておらず、**OpenAI も ChatGPT も一度も現れません。**

### なぜこれが効くか

この系列の中心にあるのは作用素 `x ← DQx + (I − D)p` です。**それが言語モデルの
出力である**という事実は、開示文が述べている「改訂の起草を手伝った」とは
まったく別の重さを持ちます。

書かれていることは本当です。**書かれていないことがある**というだけです。
**不実記載ではなく、欠落です。**

これは撤回された内容の説明にもなります。再現できるコードのない「98.7%」、
...
三本はいずれも**自己登録のプレプリント**です。出版社も、投稿規程も、審査も、
契約もありません。**当時、開示義務は存在しませんでした。**

規範そのものは 2023 年前半から存在していましたが、それらは投稿する論文に
掛かるものです。**この項目は違反の記録ではなく、記載の欠落の記録です。**

### 経路

段階ごとの記録は [ROUTE.md](ROUTE.md) にあります。
