...
for 語 in ("論文としての価値は、真であることに尽きる。ただし、いまは条件つきである。",
           "だが、貢献は主張している。",
           "この四文は帰属の側で立たない",
           "真であることは、論文の下限であって上限ではない。",
...
_m = re.search(r"推薦を求める文面に書くこと。\*{0,2}(.)つある。", ARXIV)
...
check("二人目に断られたことを記録してある",
      ...
      and "返事が来た。断りである。" in ARXIV
      and "当てのある相手は残っていない。" in ARXIV)

# **相手と分野を書かない**（決めごと 9）。書けるのは理由の型までである。
check("断られた相手と分野を書いていない",
      "**相手は書かない**（決めごと 9）。分野の名も書かない。" in ARXIV
      and "断られたことと、挙げられた理由の型までである" in ARXIV)

# **口座の有無を条件だと思い込まない。**原典の条件はそこではない。
check("口座が条件の一部でしかないと書いてある",
      "口座は条件の一部でしかない。" in ARXIV
      and "分野が違えば、口座があっても同じ結果である" in ARXIV)

# **arXiv の様式に入力されていない。**否の票と混同しない。
check("arXiv に否の票が記録されていないと書いてある",
      "arXiv 側に否の票は記録されていない。" in ARXIV
      and "推薦の可否が非公開だという話は、ここには掛からない" in ARXIV)

# **抜けを埋めずに終わったことを、埋めなくてよいと読ませない。**
check("抜けを埋める前に終わったと書いてある",
      "抜けを埋める前に終わったのであって、埋めなくてよいと決まったのではない。" in ARXIV)

# **引用から辿る道が使えないことは、次の手を決める。**
check("引用から辿れないことを書いてある",
      "引用から辿る道は、この紙面では使えない。" in ARXIV
      and "いずれも書籍である" in ARXIV)

# **好意を評価に読み替えない。**読む人が一人いる、以上のことを書かない。
check("申し出を推薦でも評価でもないと書いてある",
      "これは推薦ではない。評価でもない。" in ARXIV
      and "そう書く以上のことを、この欄に足さない。" in ARXIV)
