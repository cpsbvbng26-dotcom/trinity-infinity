...
# **太字に依らせない**（書き方 5）。内訳の一行は、語と数だけで拾う。
m3 = re.search(r"\*{0,2}((?:[^\s*、。]+ \d+、)*[^\s*、。]+ \d+)。", tbl)
...
m2 = re.search(r"([一二三四五六七八九十])段階を経て", ROUTE)
...
# **新規性を主張していない以上、残る価値は真であることだけである。**
# ここが消えると、「新規でない」から「価値が無い」へ滑る余地ができる。
for 語 in ("論文としての価値は、真であることに尽きる。ただし、いまは条件つきです。",
           "だが、貢献は主張しています。",
           "この四文は帰属の側で立ちません",
           "真であることは、論文の下限であって上限ではありません。",
           "下限と上限が一致しているとは、現在形では書けません"):
    check("結論が「%s」を保っている" % 語.replace("*", "")[:24],
          語 in README[i_concl:i_three] if i_concl >= 0 else False)

for 語 in ("枠組みは残りませんでした。",
           "学部 2〜3 年の演習問題の水準です",
           "その側が、意味を\n持たないと証明されました"):
    check("結論が「%s」を保っている" % 語.replace("\n", "")[:26],
          語 in README[i_concl:i_three] if i_concl >= 0 else False)

# **題名の半分だけが当たっていた。**当たった側を持ち出すなら、
# **回顧的解釈であることと、本が撤回されていることを同じ場所に置く。**
# 片方だけ残すと、撤回した本の題名が手柄に化ける。
for 語 in ("題名も、同じ割れ方をしています。",
           "| **ロマン** | 当たっていた。理論としては残らなかった |",
           "| **フレームワーク** | 当たっていなかった。枠組みは残らなかった |",
           "着地を知ってから題名を読み直しています。",
           "**回顧的解釈・検証不能**",
           "題名の一語が当たっていたことは、本を救い",
           "`BK-001`"):
    check("結論が題名の割れ方について「%s」を保っている" % 語.replace("*", "")[:22],
          語 in README[i_concl:i_three] if i_concl >= 0 else False)
...
check("記号のどちら側が消えたかを書いてある",
      "その側が、意味を持たないと証明されました" in README)
check("枠組みが残らなかったと書いてある", "枠組みは残りませんでした" in README)

# **水準は一段ではない。**三つに割ったことと、順位を軸にしないことを保つ。
for 語 in ("残ったものは、学部 2〜3 年の演習問題です。",
           "直しに使った道具は、大学院の線形システム論と行列解析に属します。",
           "### 大学院に入る部分",
           "非正規行列の過渡的な増大",
           "この山の高さは\n扱いません",
           "これで全体が大学院の水準になるわけではありません",
           "道具の水準と、直された中身の水準は\n別です",
           "それで全体の水準が上がるわけではありません",
           "### どの学科で扱うか",
           "いちばん近いのは三つめです",
           "世界大学ランキングは、この件の軸になりません",
           "確かめようも覆しようもない"):
...
check("結果が出ていないと書いてある", "結果はまだ出ていません。" in ARXIV)
check("新規性が無いという理由は決定で変わらないと書いてある",
      "**変わらない**" in ARXIV
      and "出すと決めたことと、新しい結果があることは別です" in ARXIV)
check("判定を取り下げるためではないと書いてある",
      "その判定を取り下げるために書いたものではありません" in ARXIV)
check("残った事実の水準が、結論と同じ語で書いてある",
      "学部 2〜3 年の演習問題" in ARXIV and "学部 2〜3 年の演習問題" in README)
check("凍結された PDF を組み直さないと書いてある", "arXiv 用に組み直しません" in ARXIV)
check("推薦者に内容を大きく見せないと書いてある",
      "推薦を求める相手に、内容を大きく見せません" in ARXIV)
check("推薦の文面に新規性と AI の関与を書くと決めてある",
      "新規の結果ではないこと。" in ARXIV
      and "初稿が言語モデルの生成物であること。" in ARXIV)
check("却下の見込みを先に書いてある", "却下されると見ています。" in ARXIV)
check("先に書いたことを日付で確かめられると書いてある",
      "この行は結果が出る前に書きました。" in ARXIV
      and "コミットの日付で確かめられます" in ARXIV)
check("通っても新規性の証明にならないと書いてある",
      "通ったことは新規性の証明になりません" in ARXIV and "arXiv は査読ではありません" in ARXIV)
check("結果を消さないと書いてある", "どの結果でも消しません。" in ARXIV)
...
check("結論が、決定で判定が変わらないことを保っている",
      "新しい結果が無いという上の一行は、その決定によって変わりません。" in README)

# **どれを出すかと、どこへ出すか。**選び方の基準は「その紙面が既知の誤りを運ぶか」である。
# 基準が消えると、選択の理由が「都合のよいほう」に置き換わる。
check("三篇のうち一篇を出すと決めてある",
      "採るのは B です。" in ARXIV and "C（新しい短報を書く）は採りません" in ARXIV)
check("選んだのが改訂版 Series I であると書いてある",
      "### 三篇のうちどれか —— 改訂版 Series I" in ARXIV)
check("選ぶ基準が書いてある",
      "選ぶ基準は、その紙面が既知の誤りを運ぶかどうかです。" in ARXIV
      and "既知の誤った参照を載せた紙面を、\n新しい場へ持ち込みません" in ARXIV)
check("一般形が Series III にあることを隠していない",
      "いちばん一般の形（任意の n ≥ 2）は Series III にあります" in ARXIV
      and "一般性のために、誤りを運ぶ紙面を選びません。" in ARXIV)
check("E1 をコメント欄に自分で書くと決めてある",
      "E1 のことは、投稿のコメント欄に自分で書きます。" in ARXIV)
...
_m = re.search(r"推薦を求める文面に書くこと。\*{0,2}(.)つあります。", ARXIV)
...
check("送り方の決めが、原典の文言に合わせてある",
      "大人数へ一斉に送りません。同じ相手へ繰り返し送りません。" in ARXIV
...
check("二人目への依頼を、相手を書かずに記録してある",
      "2026-09-18、二人目に一通を送りました。" in ARXIV
      and "**送り先は書かない**（決めごと 9）" in ARXIV
      and "計 3 通、相手は 2 人になりました" in ARXIV)
check("送った一通に足りないものを、足りないと書いてある",
      "四つめから六つめが抜けています。" in ARXIV
      and "補う一通が要ります" in ARXIV)

# **止まった段階も残す。**断られたことを書かなければ、依頼が宙に浮いたままに見える。
check("二人目に断られたことを記録してある",
      "### 二人目は断られた —— 2026-09-18" in ARXIV
      and "返事が来ました。断りです。" in ARXIV
      and "当てのある相手は残っていません。" in ARXIV)

# **相手と分野を書かない**（決めごと 9）。書けるのは理由の型までである。
check("断られた相手と分野を書いていない",
      "**相手は書かない**（決めごと 9）。分野の名も書きません。" in ARXIV
      and "断られたことと、挙げられた理由の型までです" in ARXIV)

# **口座の有無を条件だと思い込まない。**原典の条件はそこではない。
check("口座が条件の一部でしかないと書いてある",
      "口座は条件の一部でしかありません。" in ARXIV
      and "分野が違えば、口座があっても同じ結果です" in ARXIV)

# **arXiv の様式に入力されていない。**否の票と混同しない。
check("arXiv に否の票が記録されていないと書いてある",
      "arXiv 側に否の票は記録されていません。" in ARXIV
      and "推薦の可否が非公開だという話は、ここには掛かりません" in ARXIV)

# **抜けを埋めずに終わったことを、埋めなくてよいと読ませない。**
check("抜けを埋める前に終わったと書いてある",
      "抜けを埋める前に終わったのであって、埋めなくてよいと決まったのではありません。" in ARXIV)

# **引用から辿る道が使えないことは、次の手を決める。**
check("引用から辿れないことを書いてある",
      "引用から辿る道は、この紙面では使えません。" in ARXIV
      and "いずれも書籍です" in ARXIV)

# **好意を評価に読み替えない。**読む人が一人いる、以上のことを書かない。
check("申し出を推薦でも評価でもないと書いてある",
      "これは推薦ではありません。評価でもない。" in ARXIV
      and "そう書く以上のことを、この欄に足しません。" in ARXIV)
...
check("突き合わせの内訳を散文が名乗っている",
      "四つが片付き、一つで記録のほうが間違っていました。" in ARXIV)
...
check("第一希望と第二希望を決めてある",
      "第一希望は `math.NA`、第二希望は `math.DS` とします。" in ARXIV)
# **分類は主題で決まる。**新規性の水準を区分で表そうとすると、この系列が犯したのと
# 同じ取り違えになる。直した経緯ごと保つ。
check("分類が主題で決まると書いてある",
      "分類は主題で決まる。新規性の水準を表す欄ではありません。" in ARXIV
      and "区分で新規性の無さを表そうとしません。" in ARXIV)
...
check("新規性の無さをコメント欄で述べると決めてある",
      "新しい結果が無いことは、投稿のコメント欄で述べることです" in ARXIV)
check("分類を変えられる余地を書いてある",
      "econ.TH" in ARXIV and "希望と実際の両方を書きます" in ARXIV)
check("推薦の範囲が未確認だと書いてある",
      "同じ domain かどうかは確かめていません" in ARXIV)
...
check("数学の自動推薦の日付を、全区分の改定と分けてある",
      ...
      and "数学の側の日付は 2026年1月ではなく 2025年12月です。" in ARXIV)
check("推薦者の資格の範囲を書いてある",
      "3 か月前から 5 年前までの間に出されたもの" in ARXIV
      and "出したばかりの著者は推薦できません。" in SUBMIT)
check("同梱のしくみが PDF 投稿で使えないと書いてある",
      "PDF だけの投稿では働きません" in ARXIV and "PDF だけの投稿では働きません" in SUBMIT)
check("停止の中身を、及ぶ範囲ごと書いてある",
      "**1 年の投稿停止**" in ARXIV
      and "次の投稿はまず査読のある場を通す必要があります" in ARXIV
      and "数学の区分にどう及ぶかは確かめていません。" in ARXIV)
check("出典の節があり、原文を開いていないと断ってある",
      "## 出典（検索結果から取ったもの。原文の頁は開いていない）" in ARXIV)
check("裏の取れなかった一件を名指ししてある",
      "`math.HO` の対象についての記述だけは、検索でも裏が取れませんでした。" in ARXIV)
...
check("画面の文言を確かめていないと断ってある",
      "画面の文言までは確かめていません" in SUBMIT
      and "実際の表示が違えば、実際のほうが正しい" in SUBMIT)
check("所有権の主張を先に見ると書いてある",
      "Claim Ownership をいちばん先に見ます。" in SUBMIT
...
check("省くことが欺きだと書いてある", "それは相手を欺くことです" in SUBMIT)
...
check("相手の氏名と連絡先を記録に書かないと書いてある",
      ...
      and "何通送ったか、いつ送ったか、返事があったかまでです" in SUBMIT)
check("雛形が相手の氏名を埋め込みにしていない",
      "〈氏名〉" in SUBMIT and "〈name〉" in SUBMIT)

# 送り方の作法。ここを外すと、相手ではなく arXiv の側に迷惑が出る。
check("一度に一人へ送ると書いてある",
      "一度に一人へ送ります。" in SUBMIT and "スパムとして扱われます" in SUBMIT)
check("運営に頼まないと書いてある", "arXiv の運営に頼みません。" in SUBMIT)
check("承認を査読と読み替えないと書いてある",
      "承認は査読ではありません。" in SUBMIT
      and "内容が認められたこととして書きません" in SUBMIT)
check("手順のコメント欄が、区分の代わりでないと書いてある",
      "**区分ではなくここで述べる**" in SUBMIT)
check("段階 1 が済んだことを、符号を書かずに記録してある",
      "この段階は済んでいます。2026年9月9日、`math.NA` の符号を受け取りました。" in SUBMIT
      and "この文書に書いてよいのは、受け取った日と分類までです。" in SUBMIT
