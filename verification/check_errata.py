...
check("E8 —— 照合が付いたことと、その書誌が書いてある",
      "## E8 — " in _doc
      and "### 照合 —— 重なりました。そして、年号が二つある" in _doc
      ...
      and "`W` を巡回置換に限った既知の収束結果です" in _doc)
...
check("E8 —— 収束の述べ方が級数形だと書いてある",
      "収束の述べ方は、同じではありません。" in _doc
      and "幾何級数を直接足し上げて" in _doc
      and "ノイマン級数です。" in _doc
      and "矢印の向きも、二枚で逆です" in _doc)
...
check("E8 —— その節のことを正誤表が書いている",
      "Series II 第1.1節の題は `Position in the Literature` です" in _doc
...
_条件 = all(x in _rd for x in (
    "論文としての価値は、真であることに尽きる。ただし、いまは条件つきです。",
    "だが、貢献は主張しています。",
    "`E8` が重大度・高で立っているあいだ、この四文は帰属の側で立ちません",
...
check("E8 —— 開区間の狭さが何を買っているかを書いてある",
      "開区間の狭さは、代金と引き換えに何かを買っています。" in _doc
      and "狭さは、無条件性の購入です" in _doc)

# **「既知だった」だけでは、相手の格が分からない。**周縁の骨董と旗艦では、
# 同じ「特殊例」でも意味が違う。**格を書き、同時に何を落とした特殊例かも書く。**
check("E8 —— 相手の格を書いてある",
      "### どの程度のモデルか" in _doc
      and "**DeGroot と Friedkin–Johnsen の二つを並べて挙げる**" in _doc
      and "Social Influence Network Theory" in _doc
      and "*Science* 354(6310), 321–326" in _doc
      and "最小の追加で最大の差分です" in _doc)
check("E8 —— 格の目盛りの、正直な側も書いてある",
      "目盛りの正直な側も書きます。" in _doc
      and "社会学全域の地標ではありません" in _doc
      and "**ここは未照合である**（`他者の証言`）" in _doc)
check("E8 —— 何を落とした特殊例かを書いてある",
      "### そして、何を落とした特殊例か" in _doc
      and "各主体がちょうど一人の隣人だけを聴く輪" in _doc
...
check("E8 —— 棚が opinion dynamics だと書いてある",
      ...
      and "錨の項は、合意を壊すための装置です。" in _doc
      and "`p` が定数ベクトルのときだけです" in _doc
      and "半分だけ正しい住所です" in _doc)

# **模型の名を新しく立てない。**名前の帳簿に何も足していない側が取るものではない。
check("E8 —— 新しい模型名を立てないと書いてある",
      "### 定義の中に、新しい模型名を立てない" in _doc
      and "模型の名としては使い" in _doc
      and "模型の名としては使い" in _rd)
check("E8 —— 輪位相上の後続研究が未照合だと書いてある",
      "輪位相上の FJ を個別に扱った後続研究が既にあるかは**未照合**です" in _doc
      and "どちらに転んでも一般定理が覆っているので、定義文は変わりません" in _doc)

# **覆し方の無い判定は、判定ではなく宣告である。**E8 は系列全体についての
# 判定なので、覆る条件を書く。残余三つが、なぜその条件を満たさないかも。
check("E8 —— 覆し方と、残余三つが書いてある",
      "### 覆し方" in _doc
      and "非初等で、Friedkin–Johnsen・バナッハの不動点定理・" in _doc
      and "置換の等長性が与える厳密な等号" in _doc
      and "`n = 3` が対合でない最小の巡回であること" in _doc
      and "漸近レートが係数の相乗平均になること" in _doc)
check("E8 —— 覆らないことを自慢にしないと書いてある",
      "この「覆らない」は、主張が小さいことの裏返しでもあります。" in _doc
      and "そこは自慢にしません。" in _doc)

# **「全く同じではない」は、新規性を取り戻す道に見える。**そこを先に塞ぐ。
# 違いは二つが狭いほうへ、一つが広いほうへ。広いほうは定理にならない。
check("E8 —— 違いの向きを書いてある",
      "同じではありません。だが、違いの向きが悪い。" in _doc
      and "収束の議論は `u` が何であるか" in _doc
      and "一般化ではなく、名前の付け替えです。" in _doc
      and "「違いがあるから新規である」は、ここでは成り立ちません" in _doc)
...
check("E8 —— 原典に当たっていないと書いてある",
      ...
      and "外れるのは、本文をこちらで読んだときです" in _doc)
